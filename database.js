import { initializeApp } from "https://www.gstatic.com/firebasejs/12.3.0/firebase-app.js";
	...
	onValue(maintenanceRef, (snapshot) => 
	{
	    if (snapshot.exists()) 
		{
	        ...
	        if (!data.state)window.location.href = "./close.html";
	    }
	});
function getNow()
{
...
function deliverDatabase()
{
	if(checkForm())
	{
		...
		incrementCounter();
		set(ref(db,'requests/'+globalThis.id),{fullname:removeSpecialChars(name.value),phone:removeSpecialChars(phone.value),address:removeSpecialChars(address.value),cart:cartList,total:updateTotal()+100000,date:getNow(),driver:"-",state:"0"});
		emptyMyCart();
		hideForm();
		let lastcat=localStorage.getItem('lastcat');
